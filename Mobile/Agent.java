...
public class Agent implements Serializable, Runnable {
    ...
    protected String keyword = "";        // the keyword for inverted indexing
    ...
    // NEW METHOD: Get file map from current place
    public Hashtable<String, String> getMap(String hostname) {
	try {
	    // Access the current place's file map
	    if (place != null && place instanceof Place) {
		return ((Place) place).getLocalFileMap();
	    }
	} catch (Exception e) {
	    System.err.println("Error accessing file map: " + e.getMessage());
	}
	return new Hashtable<>();  // Return empty map if error
    // NEW METHOD: Get file map from current place
public Hashtable<String, String> getMap(String hostname) {
    try {
        // Access the current place's file map
